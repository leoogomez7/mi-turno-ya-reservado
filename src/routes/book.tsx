...
import { BARBERS, SERVICES, BUSINESS, generateSlots } from "@/lib/salon-data";
import { bookingStore, getBarber, sumDuration, sumPrice, type Customer } from "@/lib/booking-store";

export const Route = createFileRoute("/book")({
  head: () => ({
    meta: [
      { title: "Reservar turno — Atelier Barber" },
      { name: "description", content: "Reservá tu turno en pocos pasos: elegí peluquero, servicio, fecha y horario." },
    ],
  }),
  component: BookPage,
});

const STEPS = ["Datos", "Peluquero", "Servicios", "Fecha & hora", "Confirmar"] as const;
...
function BookPage() {
  ...
  const duration = sumDuration(serviceIds);
  const price = sumPrice(serviceIds);

  const canNext = useMemo(() => {
    if (step === 0) return customer.fullName.trim() && /.+@.+\..+/.test(customer.email) && customer.phone.trim().length >= 6;
  ...
  function confirm() {
    ...
    navigate({ to: "/book/success", search: { id: booking.id } as any });
  }
    ...
    <div className="min-h-screen bg-gradient-dark">
      ...
      <main className="mx-auto max-w-4xl px-6 py-10 md:py-14">
        ...
        <div className="mt-8 rounded-2xl border border-border bg-card/60 p-6 md:p-10">
          ...
          {step === 2 && <StepServices value={serviceIds} onChange={setServiceIds} hairType={customer.hairType} setHairType={(v) => setCustomer({ ...customer, hairType: v })} />}
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          {step < STEPS.length - 1 ? (
            <Button onClick={() => setStep((s) => s + 1)} disabled={!canNext} className="bg-gold text-primary-foreground hover:bg-gold-soft">
              Continuar <ChevronRight className="ml-1 h-4 w-4" />
            </Button>
          ) : (
            <Button onClick={confirm} className="h-12 bg-gold px-8 text-base font-semibold text-primary-foreground shadow-gold hover:bg-gold-soft">
              CONFIRMAR TURNO
            </Button>
          )}
        </div>
...
function StepBarber({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div>
      <SectionTitle icon={Scissors} title="Paso 2" subtitle="Elegí tu peluquero" />
      <div className="grid gap-4 sm:grid-cols-2">
        {BARBERS.map((b) => {
          ...
          return (
            ...
            >
              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-gold/30 to-gold/5 font-display text-lg font-semibold text-gold">
                {b.initials}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
                  <span>{b.years} años</span>
                  <span className="flex items-center gap-1 text-gold"><Star className="h-3 w-3 fill-current" /> {b.rating}</span>
                </div>
              </div>
...
function StepServices({
  value, onChange, hairType, setHairType,
}: {
  value: string[]; onChange: (v: string[]) => void;
  hairType?: Customer["hairType"]; setHairType: (v: Customer["hairType"]) => void;
}) {
  const toggle = (id: string) => onChange(value.includes(id) ? value.filter((x) => x !== id) : [...value, id]);
  const totalDur = sumDuration(value);
  const totalPrice = sumPrice(value);
  ...
  return (
    <div>
      ...
      <div className="grid gap-3 text-sm">
        ...
        <Row label="Peluquero" value={barber?.name ?? "-"} />
        <Row label="Servicios" value={serviceIds.map((id) => SERVICES.find((s) => s.id === id)?.name).filter(Boolean).join(", ")} />
